...
Além disso, os dados foram transformados do dataframe do pandas para arrays do numpy, e na sequência a matriz de covariáveis foi padronizada. Para serem utilizados como entradas na rede neural os arrays foram transformados em torch.tensor e separados em batch de tamanho $350$ no teste e $150$ na validação.
...
plt.legend()
plt.xlabel("Época")
plt.ylabel("Entropria Cruzada")
plt.title("Curva de aprendizado")
plt.show()
```

Podemos observar no gráfico das curvas de aprendizado da rede que a convergência ocorre rapidamente após poucas épocas, e a perda no treino e na validação possuem comportamento semelhante.
...
# percorrendo cada epoca
# starting neural network loop
for it in tqdm(range(n_epochs), desc="Ajustando o estimador de densidade via normalizing flows"):
    loss_train_array, loss_valid_array = [], []
...
Pelo gráfico das curvas de aprendizado para o Normalizing Flows nota-se que a convergência não ocorreu de forma tão rápida quanto a Mistura de Normais. As curvas são semelhantes para a perda no treino e validação porém, a perda no treino é levemente superior a da validação principalmente nas primeiras épocas.
...
\subsection{Item 2}
Fixaremos na [Figura @fig-plot-densities] os 5 valores de $X$ do conjunto de teste e plotaremos cada estimativa de densidade condicional para tais valores.
```{python}
...
Assim, é possivel observar na [Figura @fig-plot-densities] que a densidade condicional obtida via Mistura de Normais possui comportamento mais suave em relação as demais nas 5 observações. A densidade condicional estimada pelo Normalizing Flows tem comportamento mais conturbado, enquanto a densidade estimada pelo FlexCode apesar de possuir comportamento suave os valores são baixos.

\subsection{Item 3}
A seguir iremos computar os PIT values de cada metodologia e checar se para cada um dos modelos seguem uma distribuição uniforme. Essa checagem será feita tanto visualmente quanto a partir de um teste de KS. Primeiramente, calculamos os pit values:
```{python}
# Mistura de normais
...
\subsection{Item 1}
Ajustaremos a seguir os três modelos de classificação, são eles: Regressão logística com penalização lasso, KNN e Random Forest.
```{python}
# | cache: True
...
Para esse caso, o KNN foi ajustado com $K=30$ e o Random Forest com 300 árvores.
...
Assim, é possível notar que a Regressão Logística tem melhor desempenho, pois possui os menores valores de Score de Brier e log-loss, seguido de KNN e Random Forest, respectivamente.

\subsection{Item 2}
A seguir, podemos analisar o calibration plot de cada metodologia através da Figura:
```{python}
...
\subsection{Item 3}
Vamos a seguir recalibrar cada método via histogram binning. Para tal, ajustaremos cada método ao histogram binning no conjunto de calibragem separado inicialmente, assim, temos:
```{python}
...
results_transposed
```
Em geral, nota-se que para o Random Forest e KNN  melhora de performance com menores valores para o Score de Brier e log-loss considerando-se os desempenhos inciais dos métodos pré-calibração. A Regressão Logística teve ligeiro redução no seu desempenho em relação a pré-calibração, possivelmente devido a distribuição das probabilidades mais longe do valor $1$ após a calibragem.
Assim, o  Random Forest possui melhores valores em relação ao KNN e Regressão Logística.
